...
class Michelangelo extends Artists{
    //field
    ...
    //constructors
    Michelangelo(){}
    Michelangelo(String newName, String newMedium, int created, ArrayList newArray){
        name = newName;
        medium = newMedium;
        date = created;
    ...
    void display(){ //display one artwork
        System.out.format("Michelangelo's medium was %s and he finished %s in %s.\n", medium, name, date);
    }
    void askForFamous(){ //ask if user wants to see other artworks by the same artist
        System.out.println("\nWould you like to find out about Michelangelo's other famous art pieces? (Y/n)");
        String inputVal = usrInput.nextLine();
        if(inputVal.equalsIgnoreCase("Y")){
            System.out.println("These are some of Michelangelo's other famous art pieces: ");
